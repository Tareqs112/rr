/**
 * اختبارات تكاملية لنظام تأجير السيارات
 * يستخدم هذا الملف لاختبار وظائف النظام بشكل تكاملي
 */
...
const { createTestData, cleanDatabase } = require('./test-data');
...
const runIntegrationTests = async () => {
  try {
    ...
    // تنظيف قاعدة البيانات وإنشاء بيانات الاختبار
    await cleanDatabase();
...
const testBalanceCalculation = async () => {
  try {
    ...
    // التحقق من صحة حساب الرصيد
    return Math.abs(parseFloat(customer.balance) - expectedBalance) < 0.01; // للتعامل مع فروق التقريب
  } catch (error) {
...
module.exports = {
  ...
  testCompanyCustomerRelation,
  testPaymentSystem,
  testHotelFlightBooking,
  testTourPackage,
  testVehicleTypes
};
